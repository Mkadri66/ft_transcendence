// frontend/src/scripts/game/pong.ts
export type Unmount = () => void;

export function mountLocalPong(root: HTMLElement, opts?: { p1Name?: string; p2Name?: string; onEnd?: (res: { p1: string; p2: string; s1: number; s2: number; winner: string; }) => void }): Unmount {
  ...
  const onKeyDown = (e: KeyboardEvent) => {
    ...
    if (['w', 's', 'W', 'S', 'ArrowUp', 'ArrowDown', ' ', 'p', 'P'].includes(k)) {
      keys.add(k);
    }
  };
  ...
  function update() {
    ...
    if (ballY - BALL_R <= 0 && ballVY < 0) { ballY = BALL_R; ballVY *= -1; }
    if (ballY + BALL_R >= H && ballVY > 0) { ballY = H - BALL_R; ballVY *= -1; }

    const p1x = 20;
    if (ballX - BALL_R <= p1x + PADDLE_W && ballX - BALL_R >= p1x) {
      if (ballY >= p1y && ballY <= p1y + PADDLE_H && ballVX < 0) {
        ballX = p1x + PADDLE_W + BALL_R;
        ballVX = Math.min(Math.abs(ballVX) + 0.5, BALL_SPEED_MAX);
        const rel = (ballY - (p1y + PADDLE_H / 2)) / (PADDLE_H / 2);
        ballVY = rel * Math.max(Math.abs(ballVX) * 0.8, 3);
      }
    }

    const p2x = W - 20 - PADDLE_W;
    if (ballX + BALL_R >= p2x && ballX + BALL_R <= p2x + PADDLE_W) {
      if (ballY >= p2y && ballY <= p2y + PADDLE_H && ballVX > 0) {
        ballX = p2x - BALL_R;
        ballVX = -Math.min(Math.abs(ballVX) + 0.5, BALL_SPEED_MAX);
    ...
    if (ballX < -BALL_R) { score2++; resetBall(true); }
    if (ballX > W + BALL_R) { score1++; resetBall(false); }

    if (keys.has(' ') || keys.has('p') || keys.has('P')) {
      paused = !paused;
      keys.delete(' '); keys.delete('p'); keys.delete('P');
    }

    if (score1 >= SCORE_TO_WIN || score2 >= SCORE_TO_WIN) {
      paused = true;
      const winner = score1>score2? p1Name : p2Name;
      try { opts?.onEnd?.({ p1: p1Name, p2: p2Name, s1: score1, s2: score2, winner }); } catch(_){}
    }
  }
  ...
  function draw() {
    ...
    if (paused) {
      ...
      ctx.fillText('Pause - Espace ou P pour reprendre', W / 2, H / 2 - 10);
      ctx.fillText('Premier à 11 gagne', W / 2, H / 2 + 20);
      if (score1>=SCORE_TO_WIN || score2>=SCORE_TO_WIN){
        ctx.fillText(`Vainqueur: ${score1>score2?p1Name:p2Name}`, W/2, H/2+50);
      }
    }
  }
